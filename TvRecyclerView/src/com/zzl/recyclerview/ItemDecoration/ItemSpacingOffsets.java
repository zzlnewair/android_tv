package com.zzl.recyclerview.ItemDecoration;
...
import com.zzl.recyclerview.Lanes;
import com.zzl.recyclerview.Lanes.LaneInfo;
import com.zzl.recyclerview.SpannableGridLayoutManager;
import com.zzl.recyclerview.StaggeredGridLayoutManager;
import com.zzl.recyclerview.TvLayoutManager.Direction;
...
public class ItemSpacingOffsets {
    ...
    // add by zzl
    public int getSpacingSize(BaseLayoutManager lm) {
        if(null != lm) {
            if(lm.isVertical()) {
     ...
     */
    public boolean isSecondLane(BaseLayoutManager lm, int itemPosition, int lane) {
     ...
     */
    public static boolean isFirstChildInLane(BaseLayoutManager lm, int itemPosition) {
     ...
     */
    public static boolean isLastChildInLane(BaseLayoutManager lm, int itemPosition, int itemCount) {
        final int laneCount = lm.getLanes().getCount();
        Log.d("ItemSpacingOffsets", "isLastChildInLane...itemPosition=" + itemPosition + " , itemCount=" + itemCount + " , laneCount=" + laneCount);
    ...
    public void getItemOffsets(Rect outRect, int itemPosition, RecyclerView parent) {
        ...
        if (firstLane) {
            ...
            laneOffsetStart = (int) (laneSpacing * 0.65);
        }
        else if (secondLane && !lastLane) {
            laneOffsetStart = (int) (laneSpacing * 0.35);
        } 
        else {
        ...
        if (lastLane) {
            ...
            laneOffsetEnd = (int) (laneSpacing * 0.65);
        }
        else if (beforeLastLane && !firstLane) {
            laneOffsetEnd = (int) (laneSpacing * 0.35);
        } 
        else {
