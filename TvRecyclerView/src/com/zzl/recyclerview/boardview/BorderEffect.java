package com.zzl.recyclerview.boardview;
...
import com.zzl.recyclerview.boardview.BorderView.Effect;
...
public class BorderEffect implements Effect {
	...
	protected List<FocusListener> mFocusListener = new ArrayList<FocusListener>(
			1);
	protected List<Animator.AnimatorListener> mAnimatorListener = new ArrayList<Animator.AnimatorListener>(
			1);
	...
	public FocusListener absListViewFocusListener = new FocusListener() {
		...
		public void onFocusChanged(View oldFocus, View newFocus) {
			...
			try {
										...
										.addOnLayoutChangeListener(new View.OnLayoutChangeListener() {
											...
											public void onLayoutChange(View v,
												...
												try {
													...
													Rect rect = new Rect();
													View firstView = (View) absListView
															.getSelectedView();
													firstView
															.getLocalVisibleRect(rect);
													if (Math.abs(rect.left
															- rect.right) > firstView
															.getMeasuredWidth()) {
														factorX = (Math
																.abs(rect.left
																		- rect.right) - firstView
																.getMeasuredWidth()) / 2 - 1;
														factorY = (Math
																.abs(rect.top
																		- rect.bottom) - firstView
																.getMeasuredHeight()) / 2;
													}
													List<Animator> animatorList = new ArrayList<Animator>(
															3);
													animatorList
															.addAll(getScaleAnimator(
																	firstView,
																	true));
													animatorList
															.addAll(getMoveAnimator(
																	firstView,
																	factorX,
																	factorY));
													mTarget.setVisibility(View.VISIBLE);
				...
				} else if (oldFocus instanceof AbsListView
						&& newFocus instanceof AbsListView) {
					if (attacheViews.indexOf(oldFocus) >= 0
							&& attacheViews.indexOf(newFocus) >= 0) {
						...
						if (Math.abs(rect.left - rect.right) > firstView
								.getMeasuredWidth()) {
							factorX = (Math.abs(rect.left - rect.right) - firstView
									.getMeasuredWidth()) / 2 - 1;
							factorY = (Math.abs(rect.top - rect.bottom) - firstView
									.getMeasuredHeight()) / 2;
						}

						List<Animator> animatorList = new ArrayList<Animator>(3);
						animatorList.addAll(getScaleAnimator(firstView, true));
						animatorList.addAll(getScaleAnimator(
								a.getSelectedView(), false));

						animatorList.addAll(getMoveAnimator(firstView, factorX,
								factorY));
						mTarget.setVisibility(View.VISIBLE);
	...
	protected List<Animator> getScaleAnimator(View view, boolean isScale) {
		...
		try {
			...
			ObjectAnimator scaleX = new ObjectAnimator().ofFloat(view,
					"scaleX", scaleBefore, scaleAfter);
			ObjectAnimator scaleY = new ObjectAnimator().ofFloat(view,
					"scaleY", scaleBefore, scaleAfter);
	...
	protected List<Animator> getMoveAnimator(View newFocus, int factorX,
			int factorY) {
		...
		try {
			...
			if (mScalable) {
				...
				newXY[0] = (int) (newXY[0] - (newWidth - newFocus
						.getMeasuredWidth()) / 2.0f) + factorX;
				newXY[1] = (int) (newXY[1]
						- (newHeight - newFocus.getMeasuredHeight()) / 2.0f
						+ 0.5 + factorY);
			...
			PropertyValuesHolder valuesWithdHolder = PropertyValuesHolder
					.ofInt("width", oldWidth, newWidth);
			PropertyValuesHolder valuesHeightHolder = PropertyValuesHolder
					.ofInt("height", oldHeight, newHeight);
			PropertyValuesHolder valuesXHolder = PropertyValuesHolder.ofFloat(
					"translationX", oldXY[0], newXY[0]);
			PropertyValuesHolder valuesYHolder = PropertyValuesHolder.ofFloat(
					"translationY", oldXY[1], newXY[1]);
			final ObjectAnimator scaleAnimator = ObjectAnimator
					.ofPropertyValuesHolder(mTarget, valuesWithdHolder,
							valuesHeightHolder, valuesYHolder, valuesXHolder);

			scaleAnimator
					.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
						@Override
						public synchronized void onAnimationUpdate(
								ValueAnimator animation) {
							int width = (int) animation
									.getAnimatedValue("width");
							int height = (int) animation
									.getAnimatedValue("height");
							float translationX = (float) animation
...
//									+ attacheViews.indexOf(oldFocus.getParent()));
	...
	@Override
	public void onTouchModeChanged(View target, View attachView,
		...
		try {
			...
			if (mEnableTouch && isInTouchMode) {
				...
				if (lastFocus != null) {
					AnimatorSet animatorSet = new AnimatorSet();
					animatorSet
							.playTogether(getScaleAnimator(lastFocus, false));
					animatorSet.setDuration(0).start();
				}
			}
	...
	public void onAttach(View target, View attachView) {
		try {
			...
			if (target.getParent() != null
					&& (target.getParent() instanceof ViewGroup)) {
			...
			if (attachView instanceof RecyclerView) {
				...
				recyclerViewOnScrollListener = new RecyclerView.OnScrollListener() {
					...
					public void onScrollStateChanged(RecyclerView recyclerView,
						...
						try {
							...
							if (newState == RecyclerView.SCROLL_STATE_IDLE) {//SCROLL_STATE_IDLE是当屏幕停止滚动时
								// Log.d(TAG, "=====>SCROLL_STATE_IDLE");
								isScrolling = false;
								View oldFocus = oldLastFocus;
								View newFocus = lastFocus;
								VisibleScope scope = checkVisibleScope(
										oldFocus, newFocus);
								if (!scope.isVisible) {
									return;
								} else {
								...
								// list.addAll(getScaleAnimator(oldLastFocus,
								// false));
							...
							} else if (newState == RecyclerView.SCROLL_STATE_SETTLING) {//最后一个位置的过程。
								// Log.d(TAG, "========>SCROLL_STATE_SETTLING="
								// + mAnimatorSet.isRunning());
								isScrolling = true;
								if (lastFocus != null) {
									List<Animator> list = getScaleAnimator(
											lastFocus, false);
									AnimatorSet animatorSet = new AnimatorSet();
				...
				onItemSelectedListenerList.put(attachView,
	...
	protected class MyOnItemSelectedListener implements
		...
		public void onItemSelected(AdapterView<?> parent, View view,
			...
			try {
				...
				// Log.d(TAG, "onItemSelected:" + vg.getWidth() + " " +
				// newFocus.getMeasuredWidth() + " w=" + (rect.left -
				// rect.right));

				int factorX = 0, factorY = 0;
				if (Math.abs(rect.left - rect.right) > newFocus
						.getMeasuredWidth()) {
					factorX = (Math.abs(rect.left - rect.right) - newFocus
							.getMeasuredWidth()) / 2 - 1;
					factorY = (Math.abs(rect.top - rect.bottom) - newFocus
							.getMeasuredHeight()) / 2;
				...
				animatorList
						.addAll(getMoveAnimator(newFocus, factorX, factorY));
				mTarget.setVisibility(View.VISIBLE);
